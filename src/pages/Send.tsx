import AmountConverter from '@/components/AmountTransaction';
import BeneficiaryForm from '@/components/BeneficaireForm';
import MainLayout from '@/components/layouts/MainLayout';
import NetworkSelector from '@/components/NetworkTransaction';
import CountrySelect from '@/components/Send';
import TransferSummary from '@/components/TransactionSum';
import { useGetCountries } from '@/hooks/useCountry';
import { getRate } from '@/lib/country';
...
const SendPage: React.FC = () => {
  const { state } = Auth();
  const { countries } = useGetCountries('send');
  const router = useNavigate();
  // const { rate } = useGetRateByCode();

  const [selectedCountry, setSelectedCountry] = useState<
    (typeof COUNTRIES)[0] | null
  >(COUNTRIES.find(country => country.id !== state.user?.Country.name) ?? null);
  ...
  useEffect(() => {
    ...
    const fetchRatesAndNetworks = async () => {
      if (!state.user || !selectedCountry) return;

      const codeTo = getCountryToCode(selectedCountry);

      setCountryData(
        getCountryById(countries as ICountry[], codeTo) as ICountry
      );

      try {
        const code = `${state.user.Country.name}-${codeTo}`;
      ...
      try {
        const myCt = countries?.find(el => el.name === codeTo) as ICountry;
  ...
  }, [selectedCountry, countries, state]);
  ...
  const createTransactions = useCallback(
    async (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
      ...
      if (
        ...
        infoMessage(
          `le montant à envoyer doit être compris entre ${
            (rate?.intervalMin || '') + ' ' + state.user?.Country.currency
          } et ${(rate?.intervalMax || '') + ' ' + state.user?.Country.currency}`
        );
      ...
      if (
        parseInt(
          countries?.find(el => el.name === selectedCountry?.id)
            ?.TelMaxNumber || '0'
        ) !== phone.length
      ) {
      ...
      const transaction: ITrasanctionData = {
        ...
        clientEmail: state.user?.email as string,
        fees: fee,
        networkId: netId?.id as string,
        type: 'send',
        receiverPhone: phone as string,
        receiverName: name,
        code: `${state.user?.Country.name}-${codeTo}`,
      ...
      <div className='transfert__container'>
        <div className='transfert__wrapper'>
          <div className='transfert__content'>
            <CountrySelect
              countries={COUNTRIES}
              userCountryId={state.user?.Country.name as string}
            ...
            {loading ? (
              ...
              <>
                <AmountConverter
                  ...
                  rate={rate as IRate}
                  userCurrency={state.user?.Country.currency as string}
                ...
                <BeneficiaryForm
                  ...
                  countryList={countries as ICountry[]}
                  selectedCountry={selectedCountry}
                />
                <NetworkSelector
                  networks={networks}
